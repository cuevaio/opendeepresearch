...
export const evaluateSearchResult = schemaTask({
	...
	async run({ query, searchResult, accumulatedSources }) {
		const { object } = await generateObject({
			...
			prompt: `Current date: ${new Date().toISOString()}. Evaluate whether the search result is relevant and will help answer the following query: ${query}. If the page already exists in the existing results, mark it as irrelevant. If the result is not relevant, provide an alternative query that might be more relevant.
